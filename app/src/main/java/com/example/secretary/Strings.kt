...
object Strings {
    ...
    val gcalConnect get() = t("Connect Google Calendar", "Připojit Google Kalendář", "Połącz")
    val gcalDisconnect get() = t("Disconnect", "Odpojit", "Odłącz")
    val gcalChooseCalendar get() = t("Calendar to sync:", "Kalendář pro synchronizaci:", "Kalendarz:")
    val gcalSyncNow get() = t("Sync now", "Synchronizovat teď", "Synchronizuj")
    val gcalSyncDone get() = t("Sync started.", "Synchronizace spuštěna.", "Synchronizacja uruchomiona.")
    val gcalSyncFailed get() = t("Sync failed.", "Synchronizace selhala.", "Synchronizacja nieudana.")
    val gcalSaved get() = t("Saved.", "Uloženo.", "Zapisano.")
    val gcalRefresh get() = t("Refresh", "Obnovit", "Odśwież")
    val gcalAfterConnect get() = t("Approve access in the browser, then tap Refresh.", "Povol přístup v prohlížeči, pak ťukni na Obnovit.", "Zatwierdź w przeglądarce, potem Odśwież.")
    ...
    val noRateTypesConfigured get() = t("No rate types configured yet. Set them up in Settings.", "Zatím nejsou nastaveny žádné typy sazeb. Nastavte je v Nastavení.", "Brak skonfigurowanych typów stawek. Ustaw je w Ustawieniach.")
    val addCustomRateType get() = t("Add rate type", "Přidat typ sazby", "Dodaj typ stawki")
    val rateTypeName get() = t("Rate type name", "Název typu sazby", "Nazwa typu stawki")
    val defaultRate get() = t("Default rate (£)", "Výchozí sazba (£)", "Domyślna stawka (£)")
    val rateTypeKeyHint get() = t("Internal key", "Interní klíč", "Klucz wewnętrzny")
    val enterRateTypeNameError get() = t("Enter a name for this rate type", "Zadejte název typu sazby", "Wprowadź nazwę typu stawki")
    val deleteRateType get() = t("Delete rate type", "Smazat typ sazby", "Usuń typ stawki")
    val confirmDeleteRateType get() = t("Delete rate type", "Smazat typ sazby", "Usuń typ stawki")
    val ratesLoadedFromServer get() = t("Rates loaded from server", "Sazby načteny ze serveru", "Stawki załadowane z serwera")
    val add get() = t("Add", "Přidat", "Dodaj")
    val saved get() = t("Saved", "Uloženo", "Zapisano")
