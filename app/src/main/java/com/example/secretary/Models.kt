...
data class FirstInstallRequest(
    ...
    // Catalogue codes — backend owns the catalogue; Android sends codes only
    val selected_industries: List<String> = emptyList(),
    val selected_subtypes: List<String> = emptyList(),
    val selected_activities: List<String> = emptyList(),
    val primary_industry: String? = null,
    val primary_subtype: String? = null,
...
// Matches server secretary_clean catalogue: Industry → WorkSubtype → WorkActivity
...
data class CatalogueIndustrySubtype(
    ...
    val name: String = "",
    val display_order: Int = 0,
    val activities: List<CatalogueWorkActivity> = emptyList()
)

data class CatalogueWorkActivity(
    val code: String = "",
    val name: String = "",
    val default_pricing_method_code: String = "",
    val available_pricing_method_codes: List<String> = emptyList()
)
...
enum class ConnectionStatus { UNKNOWN, CONNECTED, DISCONNECTED, TESTING }

// ── Password reset / recovery models ────────────────────────────────────────
