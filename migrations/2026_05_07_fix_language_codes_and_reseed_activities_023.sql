-- Migration 023: Fix missing unique indexes + normalize language codes + fix industry names
-- Root cause: uq_industry_subtypes_group_code index missing causes all ON CONFLICT to fail

BEGIN;

-- 1. Create missing unique indexes (these are required for ON CONFLICT to work)
CREATE UNIQUE INDEX IF NOT EXISTS uq_industry_groups_code
    ON crm.industry_groups (code);

CREATE UNIQUE INDEX IF NOT EXISTS uq_industry_subtypes_group_code
    ON crm.industry_subtypes (industry_group_id, code);

-- 2. Normalize language codes in tenant_languages
UPDATE crm.tenant_languages SET language_code = 'cs-CZ'
    WHERE language_code IN ('cs','cs_CZ','cs_cz');
UPDATE crm.tenant_languages SET language_code = 'pl-PL'
    WHERE language_code IN ('pl','pl_PL','pl_pl');
UPDATE crm.tenant_languages SET language_code = 'de-DE'
    WHERE language_code IN ('de','de_DE','de_de');
UPDATE crm.tenant_languages SET language_code = 'fr-FR'
    WHERE language_code IN ('fr','fr_FR','fr_fr');
UPDATE crm.tenant_languages SET language_code = 'sk-SK'
    WHERE language_code IN ('sk','sk_SK','sk_sk');
UPDATE crm.tenant_languages SET language_code = 'hu-HU'
    WHERE language_code IN ('hu','hu_HU','hu_hu');
UPDATE crm.tenant_languages SET language_code = 'ro-RO'
    WHERE language_code IN ('ro','ro_RO','ro_ro');
UPDATE crm.tenant_languages SET language_code = 'uk-UA'
    WHERE language_code IN ('uk','uk_UA','uk_ua');
UPDATE crm.tenant_languages SET language_code = 'ru-RU'
    WHERE language_code IN ('ru','ru_RU','ru_ru');
UPDATE crm.tenant_languages SET language_code = 'en-GB'
    WHERE language_code IN ('en','en_GB','en_gb');

-- 3. Normalize tenant_operating_profile defaults
UPDATE crm.tenant_operating_profile SET
    default_internal_language_code = CASE default_internal_language_code
        ...
        WHEN 'de' THEN 'de-DE' WHEN 'sk' THEN 'sk-SK' ELSE default_internal_language_code END,
    default_customer_language_code = CASE default_customer_language_code
        ...
        ELSE default_customer_language_code END;

-- 4. Fix stale industry group names
...
-- 5. Ensure landscaping subtype exists under trades (now safe with unique index above)
